...
void ArcBall::beginDrag(const glm::vec2& msc)
{
  ...
  mVDown = { (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0f, 1.0)).x,
      (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0f, 1.0)).y,
      (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0f, 1.0)).z };
}
...
void ArcBall::drag(const glm::vec2& msc)
{
  // Regular drag code to follow...
    mVNow = { (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0, 1.0)).x,
        (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0, 1.0)).y,
        (mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0, 1.0)).z
    };
